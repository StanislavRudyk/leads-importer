...
from .normalizer import (
    normalize_email, normalize_phone, normalize_city,
    normalize_country, normalize_state, normalize_nationality,
    extract_file_date, extract_context_from_path,
)
from .merger import (
    merge_lead_fields, deduplicate_batch, build_import_history_entry,
)
...
def _normalize_row(raw_row: Dict[str, Any], source_name: str) -> Optional[Dict[str, Any]]:
                ...
                if c_lets >= 3 and c_digs == 0:
                    ...
                    if cand_lower not in NOT_CITIES:
                        ...
                        if test_city and test_country:
                            ...
                            break
    if city_name and (city_name.lower() in NOT_CITIES or any(c.isdigit() for c in city_name)): city_name = None
    if not country_iso2 and city_country: country_iso2 = city_country
    ...
    return {
        'email': email, 'phone': phone_e164, 'phones': [phone_e164] if phone_e164 else [],
        'first_name': _clean_name(raw_row.get('first_name')), 'last_name': _clean_name(raw_row.get('last_name')),
        'country_iso2': country_iso2, 'nationality': normalize_nationality(raw_row.get('nationality')),
        'city': city_name, 'state': state, 'language': _clean_string(raw_row.get('language')),
        'latest_source': source_name, 'latest_campaign': None, 'status': 'new',
        'is_buyer': _parse_boolean(raw_row.get('is_buyer')), 'tags': _parse_tags(raw_row.get('tags')),
        'meta_info': {'import_history': [import_entry], **({'raw_phones': [raw_phone_str]} if raw_phone_str and not phone_e164 else {})},
        'file_created_at': file_date, '_file_date': file_date, 'import_count': 1,
    }
...
async def run_import(file_path: str, source_name: str = 'default', notify: bool = True, on_progress=None) -> Dict[str, Any]:
    ...
    if not os.path.exists(file_path): return {'status': 'error', 'message': f'File not found', 'duration': 0}
    import re
    ...
    if on_progress: await on_progress(0, 0, 0, phase="parsing", total=0)
    t0 = time.time()
    raw_rows = await parse_file_bulk(file_path, source_name)
    parse_time = round(time.time() - t0, 2)
    total_parsed = len(raw_rows)
    if on_progress: await on_progress(0, 0, 0, phase="parsed", total=total_parsed)
    if total_parsed == 0:
    ...
    t_bulk = time.time()
    total_inserted, total_updated, total_skipped_db, total_skipped_norm = 0, 0, 0, 0
    CHUNK_SIZE = 1000 
    async with AsyncSessionLocal() as session:
        for i in range(0, total_parsed, CHUNK_SIZE):
            ...
            if not normalized_chunk: continue
            try:
                ins, upd, skip = await _flush_batch(session, normalized_chunk, source_name)
                total_inserted += ins
                total_updated += upd
                total_skipped_db += skip
                if on_progress: await on_progress(total_inserted, total_updated, total_skipped_norm + total_skipped_db, phase="saving", total=total_parsed)
            except Exception as exc: logger.error(f'Batch error at index {i} [{filename}]: {exc}')
    db_time, duration = round(time.time() - t_bulk, 2), round(time.time() - start_time, 2)
    total_skipped = total_skipped_norm + total_skipped_db
    if total_parsed > 0 and total_inserted + total_updated == 0: status = 'skipped'
    else: status = 'failed' if total_inserted + total_updated == 0 and total_parsed > 0 else ('partial' if total_skipped > total_parsed * 0.5 else 'success')
    async with AsyncSessionLocal() as session:
    ...
    if notify:
        ...
        except Exception as exc: logger.error(f'Error sending import summary [{filename}]: {exc}')
    return results
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--source', default=None)
